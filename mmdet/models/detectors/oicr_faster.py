...
class OICR(BaseDetector):
    ...
    def forward_train(self,
        ...
        if self.with_rpn:
            ...
            print(proposal_list[0].requires_grad)

        if strong_label.any():
            roi_losses_branch1 = self.roi_head_branch1.forward_train(x, img_metas, proposal_list,
                                                                                           gt_bboxes, gt_labels,strong_label,
                                                                                           gt_bboxes_ignore, gt_masks,
                                                                                           **kwargs)
            roi_losses_branch2 = self.roi_head_branch2.forward_train(x, img_metas, proposal_list,
                                                                     gt_bboxes, gt_labels, strong_label,
                                                                     gt_bboxes_ignore, gt_masks,
                                                                     **kwargs)
        else:
            roi_losses_branch1,oam_bboxes,oam_labels = self.roi_head_branch1.forward_train(x, img_metas, proposal_list,
                                                 gt_bboxes, gt_labels,strong_label,
                                                 gt_bboxes_ignore, gt_masks,
                                                         **kwargs)

            oam_labels[0]=oam_labels[0].cuda()
            roi_losses_branch2 = self.roi_head_branch2.forward_train(x, img_metas, proposal_list,
                                                                 oam_bboxes, oam_labels,strong_label,
                                                                 gt_bboxes_ignore, gt_masks,
                                                                 **kwargs)


        losses.update(roi_losses_branch1)

        losses.update(roi_losses_branch2)
        # print(losses)
        return losses
